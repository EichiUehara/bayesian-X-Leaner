...
Clean-data efficiency of robust nuisance
    ...
    & Sec.~\ref{sec:experiments:ihdp} \\
Basis sensitivity
    & User supplies $\tx$ form; wrong basis $\to$ $\PEHE$ degradation
    & Sec.~\ref{sec:experiments:tail_signal} \\
Compute
...
§\ref{sec:experiments:other_paradigms} can be used for finer
tuning.
...
Section~\ref{sec:experiments:efficiency} is not closable by relaxing
...
Section~\ref{sec:experiments:hillstrom}).
\textbf{(d) Auto-severity breaks at high contamination.} The Hill-
estimator-based severity selector
(Section~\ref{sec:experiments:auto_severity}) routes light/moderate
...
result for the modular-Bayes posterior of Section~\ref{sec:experiments:nuisance_bootstrap} that propagates nuisance
...
variance match we use in §\ref{sec:experiments:learning_rate}.
...
in Section~\ref{sec:experiments:coverage} and find that
...
(Eq.~\ref{eq:eta_a}) should replace the trace formula.
...
(Section~\ref{sec:experiments:ihdp}, $0.683 \pm 0.23$) succeeds and
...
diagnostic of Section~\ref{sec:experiments:coverage} tells the user
...
defaults table of §\ref{sec:discussion:defaults}):
...
(§\ref{sec:experiments:basis_ablation}).
...
(Section~\ref{sec:experiments:hetero_evt_phase3}) show the concept
...
Section~\ref{sec:experiments:auto_severity} is a first step, but
its failure at high contamination (§\ref{sec:experiments:auto_severity})
...
calibration argument of Section~\ref{sec:method:phase3} with a
