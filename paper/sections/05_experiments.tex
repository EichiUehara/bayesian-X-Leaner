...
(Section~\ref{sec:experiments:efficiency}).
...
5 reps, the lowest mean among all estimators tested. We extended
...
Section~\ref{sec:experiments:coverage} provides the empirical
...
Two observations. First, the crossover between the clean-data
...
efficiency result: a less aggressive redescender pays less
efficiency cost on clean data. But the gap to XGB-MSE (0.56)
...
Three findings, two positive and one negative. \textbf{(i) A
tail-aware basis is the correct tool for tails-as-signal}
(extended basis-sensitivity analysis in
Section~\ref{sec:experiments:basis_ablation}). With
...
Section~\ref{sec:experiments:tail_signal} is binary; a reviewer may
...
boundary of Section~\ref{sec:discussion} concrete: if the
...
(§\ref{sec:experiments:nuisance_bootstrap},
§\ref{sec:experiments:coverage}) are reported at 30 seeds with
...
The IHDP comparison (Section~\ref{sec:experiments:ihdp}) shows
...
all three challenges only RX-Welsch+severity="severe" is operationally
...
(Section~\ref{sec:experiments:nuisance_bootstrap}) restores nominal
...
§\ref{sec:experiments:learning_rate} are confirmed at $p = 21$ and
...
of §\ref{sec:experiments:nuisance_bootstrap} rather than
...
§\ref{sec:experiments:nuisance_bootstrap} ($M = 8$ Bayesian-bootstrap
...
calibration problem the rest of §\ref{sec:experiments:coverage}
focuses on.
...
$Y_0$ noise from a Lévy $\alpha$-stable distribution at
...
the Hill-estimator auto-severity (§\ref{sec:experiments:auto_severity})
...
Confirms the §\ref{sec:related:tails} taxonomy: shifting the
...
$\delta$-mapping issue we flagged in §\ref{sec:method:phase1}:
...
Beyond the marginal random-W placebo (§\ref{sec:experiments:hillstrom}),
...
documented in §\ref{sec:experiments:whale}: 40\% subgroup coverage
...
on a step function. The conclusion of §\ref{sec:experiments:basis_ablation}
...
§\ref{sec:experiments:basis_ablation} is a Bayesian variable-selection
...
than the BMA softmax (§\ref{sec:experiments:basis_ablation},
...
The basis-BMA result of §\ref{sec:experiments:basis_ablation} uses
...
work in §\ref{sec:discussion:future}.
...
The brittleness exposed in §\ref{sec:experiments:basis_ablation}
...
§\ref{sec:experiments:basis_ablation}, with the true threshold
...
contaminated-normal mixture) suffice? Section~\ref{sec:experiments:coverage}
...
The construction sketched in §\ref{sec:method:phase3}: $M = 10$
...
§\ref{sec:method:phase3} acknowledges the Welsch posterior is a
...
§\ref{sec:experiments:coverage} (ATE $1.985$, width $0.29$ at the
...
§\ref{sec:experiments:coverage} (width 0.29 at 20\% density)
...
intervals (§\ref{sec:experiments:coverage}) translate into a
...
§\ref{sec:experiments:quantile_dr}: the median estimate at 1\%
