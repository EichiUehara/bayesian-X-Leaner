...
Section~\ref{sec:experiments:tail_signal} and
...
(Section~\ref{sec:method:severity}), which maps an assumed
contamination rate to Huber's minimax-optimal $\delta$
(Eq.~\ref{eq:huber_minimax}). This is the \emph{outcome-level}
...
Huber's minimax-$\delta$ values (Eq.~\ref{eq:huber_minimax}) are
...
(Section~\ref{sec:experiments:efficiency}) and the Lalonde-NSW
underperformance (Section~\ref{sec:experiments:hillstrom}): on
...
experiments of Section~\ref{sec:experiments} but they are available
...
(Section~\ref{sec:experiments:efficiency}). We use a Student-$t$
...
DGPs of Section~\ref{sec:experiments} the procedure yields
$\hat\eta^\star_\mathrm{tr} \in [0.5, 1.5]$ on clean data and
inflates modestly under contamination.
Section~\ref{sec:experiments:learning_rate} reports the LLB-bootstrap
...
contamination (§\ref{sec:experiments:coverage}) is therefore
...
§\ref{sec:experiments:coverage} confirm this: even with
...
Section~\ref{sec:experiments:learning_rate} reports the
...
between-$m$ variance. Section~\ref{sec:experiments:nuisance_bootstrap}
...
Empirically (Section~\ref{sec:experiments:nuisance_bootstrap})
...
Section~\ref{sec:experiments} for empirical evidence; an additional
...
empirical probe of Section~\ref{sec:experiments:tail_signal}
